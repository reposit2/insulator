...
# enhancer_importance_mean.txt: Average DeepLIFT score for each promoter and enhancer regulator in each tissue.
...
feature_names = read_delim(paste0(outloc,"/feature_norm_stats.txt"),
                           delim = ",",col_names = T)
...
lapply(0:0, function(sample_indx){
##lapply(4:4, function(sample_indx){
...
write.table(promoter_importance_mean,file = paste0(outloc,"/DeepLIFT/enhancer_importance_mean.txt"),
